...
{
	ip = ip | ((*block_64 & 0x1000000) >> 24 - 0);						// 40
	ip = ip | ((*block_64 & 0x100000000000000) >> 56 - 1);				// 8
	ip = ip | ((*block_64 & 0x10000) >> 16 - 2);						 // 48
	ip = ip | ((*block_64 & 0x1000000000000) >> 48 - 3);                 // 16
	ip = ip | ((*block_64 & 0x100) >> 8 - 4);							 // 56
	ip = ip | ((*block_64 & 0x10000000000) >> 40 - 5);                   // 24
	ip = ip | ((*block_64 & 0x1) >> 0 - 6);								 // 64
	ip = ip | ((*block_64 & 0x100000000) >> 32 - 7);                     // 32
	ip = ip | ((*block_64 & 0x2000000) >> 25 - 8);						 // 39
	ip = ip | ((*block_64 & 0x200000000000000) >> 57 - 9);               // 7
	ip = ip | ((*block_64 & 0x20000) >> 17 - 10);						 // 47
	ip = ip | ((*block_64 & 0x2000000000000) >> 49 - 11);                // 15
	ip = ip | ((*block_64 & 0x200) >> 9 - 12);							 // 55
	ip = ip | ((*block_64 & 0x20000000000) >> 41 - 13);                  // 23
	ip = ip | ((*block_64 & 0x2) >> 1 - 14);							 // 63
	ip = ip | ((*block_64 & 0x200000000) >> 33 - 15);                    // 31
	ip = ip | ((*block_64 & 0x4000000) >> 26 - 16);				         // 38
	ip = ip | ((*block_64 & 0x400000000000000) >> 58 - 17);	           // 6
	ip = ip | ((*block_64 & 0x40000) >> 18 - 18);				         // 46
	ip = ip | ((*block_64 & 0x4000000000000) >> 50 - 19);                  // 14
	ip = ip | ((*block_64 & 0x400) >> 10 - 20);						     // 54
	ip = ip | ((*block_64 & 0x40000000000) >> 42 - 21);                  // 22
	ip = ip | ((*block_64 & 0x4) >> 2 - 22);						     // 62
	ip = ip | ((*block_64 & 0x400000000) >> 34 - 23);                    // 30
	ip = ip | ((*block_64 & 0x8000000) >> 27 - 24);                      // 37
	ip = ip | ((*block_64 & 0x800000000000000) >> 59 - 25);                 // 5
	ip = ip | ((*block_64 & 0x80000) >> 19 - 26);					     // 45
	ip = ip | ((*block_64 & 0x8000000000000) >> 51 - 27);                   // 13
	ip = ip | ((*block_64 & 0x800) >> 11 - 28);							 // 53
	ip = ip | ((*block_64 & 0x80000000000) >> 43 - 29);                  // 21
	ip = ip | ((*block_64 & 0x8) >> 3 - 30);							 // 61
	ip = ip | ((*block_64 & 0x800000000) >> 35 - 31);                    // 29
	ip = ip | ((*block_64 & 0x10000000) >> 28 - 32);                     // 36
	ip = ip | ((*block_64 & 0x1000000000000000) >> 60 - 33);             // 4
	ip = ip | ((*block_64 & 0x100000) >> 20 - 34);				         // 44
	ip = ip | ((*block_64 & 0x10000000000000) >> 52 - 35);                 // 12
	ip = ip | ((*block_64 & 0x1000) >> 12 - 36);					     // 52
	ip = ip | ((*block_64 & 0x100000000000) >> 44 - 37);                 // 20
	ip = ip | ((*block_64 & 0x10) >> 4 - 38);						     // 60
	ip = ip | ((*block_64 & 0x1000000000) >> 36 - 39);                   // 28
	ip = ip | ((*block_64 & 0x20000000) >> 29 - 40);                     // 35
	ip = ip | ((*block_64 & 0x2000000000000000) >> 61 - 41);            // 3
	ip = ip | ((*block_64 & 0x200000) >> 21 - 42);					     // 43
	ip = ip | ((*block_64 & 0x20000000000000) >> 53 - 43);             // 11
	ip = ip | ((*block_64 & 0x2000) >> 13 - 44);					   // 51
	ip = ip | ((*block_64 & 0x200000000000) >> 45 - 45);                 // 19
	ip = ip | ((*block_64 & 0x20) >> 5 - 46);						     // 59
	ip = ip | ((*block_64 & 0x2000000000) >> 37 - 47);                   // 27
	ip = ip | ((*block_64 & 0x40000000) >> 30 - 48);                      // 34
	ip = ip | ((*block_64 & 0x4000000000000000) >> 62 - 49);               // 2
	ip = ip | ((*block_64 & 0x400000) >> 22 - 50);				        // 42
	ip = ip | ((*block_64 & 0x40000000000000) >> 54 - 51);                  // 10
	ip = ip | ((*block_64 & 0x4000) >> 14 - 52);					     // 50
	ip = ip | ((*block_64 & 0x400000000000) >> 46 - 53);                 // 18
	ip = ip | ((*block_64 & 0x40) >> 6 - 54);						     // 58
	ip = ip | ((*block_64 & 0x4000000000) >> 38 - 55);                   // 26
	ip = ip | ((*block_64 & 0x80000000) >> 31 - 56);                     // 33
	ip = ip | ((*block_64 & 0x8000000000000000) >> 63 - 57);               // 1
	ip = ip | ((*block_64 & 0x800000) >> 23 - 58);				         // 41
	ip = ip | ((*block_64 & 0x80000000000000) >> 55 - 59);                 // 9
	ip = ip | ((*block_64 & 0x8000) >> 15 - 60);						 // 49
	ip = ip | ((*block_64 & 0x800000000000) >> 47 - 61);                 // 17
	ip = ip | ((*block_64 & 0x80) >> 7 - 62);							 // 57
	ip = ip | ((*block_64 & 0x8000000000) >> 39 - 63);                      // 25
}
